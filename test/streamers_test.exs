defmodule StreamersTest do
  ...
  test "extracts m3u8 from index file" do
    ...
    assert List.first(m3u8s) ==
      Streamers.streamers_record(program_id: 1, bandwidth: 110000, path: @m3u8_sample)
    assert length(m3u8s) == 5
  end

  test "processes m3u8" do
    m3u8s = @index_file |> Streamers.extract_m3u8 |> Streamers.process_m3u8
    sample = Enum.find(m3u8s, fn(m3u8) -> Streamers.streamers_record(m3u8, :path) == @m3u8_sample end)
    assert length(Streamers.streamers_record(sample, :ts_files)) == 510
  end
end
